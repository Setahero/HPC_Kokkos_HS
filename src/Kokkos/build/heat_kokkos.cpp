...
int main(int argc, char* argv[])
{
   ...
   Kokkos::initialize(argc,argv);
	...
	for (auto i = 1; i <= iteration; ++i)
	{
	    ...
	    Kokkos::parallel_for(team_policy(x, Kokkos::AUTO),KOKKOS_LAMBDA (const member_type &teamMember)
	    {
	    	const int i = teamMember.league_rank();
	    	Kokkos::parallel_for(Kokkos::TeamThreadRange(teamMember,y), [&i,&array_new, &array] (const int j)
	    	{
